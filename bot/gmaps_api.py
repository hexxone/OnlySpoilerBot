...
from bot.weekdays import WeekModel, DayModel, HourModel

logger = logging.getLogger(__name__)
logger.info("gmaps_api loaded")
...
class MapsLocation:
    ...
    def __init__(self, location):
        ...
        self.day_index: int = datetime.today().weekday()
        self.time_index = int(datetime.now().strftime("%H"))
        self.weekday: str = self.weekdays[self.day_index]

        self.week: WeekModel = self.raw_week_to_weekmodel(raw_week)
        self.current_time: HourModel = self.html_to_current_hourmodel(html)

    def get_visited(self, day: str, hour: int):
        day_index: int = self.weekdays.index(day)
        day_info: DayModel = self.week.get_day(day_index)
        hour_info: HourModel = day_info.get_hour(hour)
    ...
    def get_current_visited(self):
        from datetime import datetime
        # day_index: int = datetime.today().weekday()
        day_info: DayModel = self.week.get_day(self.day_index)

        # get historical time info for the current situation for comparison
        statistic_hour_info: HourModel = day_info.get_hour(self.current_time.time)
        hour_info: HourModel = self.current_time
    ...
    def get_html_from_location(self, location: str) -> str:
        ...
        url: str = location_urls[location]
    ...
    def raw_week_to_weekmodel(self, raw_week: str) -> WeekModel:
        ...
        for raw_day in tokenized_days:
            ...
            if not tokenized_hours:
                week.set_day(DayModel(day_index, is_closed=True))
            else:
                for current_hour in tokenized_hours:
                    ...
                    week.get_day(day_index).set_hour(time, HourModel(time, visited, is_closed=False))

        return week

    def html_to_current_hourmodel(self, html: str) -> HourModel:
        ...
        if len(raw_current_day) == 0:
            return self.week.get_day(self.day_index).get_hour(self.time_index)
        ...
        return HourModel(int(time), visited)
...
# location = MapsLocation('fitx-adenauer')